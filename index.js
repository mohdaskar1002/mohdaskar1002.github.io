document.addEventListener('DOMContentLoaded', function() {
  ...
  let eventDate = new Date( yearOfTheEvent, 9, 23 )
  const isItFebruary4th = currentTime.getMonth() === 9 && currentTime.getDate() === 23

  function countdown() {
    const now = new Date()
    
    if (now > eventDate) {
      eventDate = new Date( yearOfTheEvent + 1, 9, 23 )
    } else if ( now.getFullYear() === eventDate.getFullYear() + 1 ) {
      eventDate = new Date( now.getFullYear(), 9, 23 )
    }
    ...
    if ( isItFebruary4th ) {
      console.log('Happy birthday, Samiya')
